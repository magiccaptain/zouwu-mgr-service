...
export class HostServerService {
  ...
  async putTextFileBySshConfig(
    ...
    const node_ssh = await this.connectBySshConfig(
      ssh_host,
      ssh_port,
      ssh_user
    );
    const temp_file_path = path.join(
      os.tmpdir(),
      `remote_text_${Date.now()}_${path.basename(remote_file)}`
    );

    try {
      const remote_dir = path.dirname(remote_file);
      await node_ssh.execCommand(`mkdir -p ${JSON.stringify(remote_dir)}`);

      fs.writeFileSync(temp_file_path, content, 'utf-8');

      await node_ssh.putFile(temp_file_path, remote_file);
    } finally {
    ...
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(() => {
          reject(
            new Error(
              `${remoteCommand.hostServer.ssh_port} 远程命令执行超时（${settings.ssh.command_timeout}ms）: ${cmd}`
            )
          );
