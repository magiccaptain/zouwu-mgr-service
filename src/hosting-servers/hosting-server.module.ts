...
import { HostingServer, HostingServerSchema } from './entities/hosting-server.entity';
import { HostingServerController } from './hosting-server.controller';
import { HostingServerService } from './hosting-server.service';

@Module({
  imports: [MongooseModule.forFeature([{ name: HostingServer.name, schema: HostingServerSchema }])],
