...
import { VdFamilyMemberStatus, VirtualDeviceService } from 'src/virtual-device';
import { Voice, VoiceService } from 'src/voice';

describe('Virtual device share (e2e)', () => {
  ...
  const mongoUrl = 'mongodb://localhost/virtual-device-share-e2e';
  ...
  it(`Device share`, async () => {
    ...
    // 邀请家庭成员
    let familyMemberResp = await request(app.getHttpServer())
      .post(`/virtual-devices/${virtualDevice.id}/family-members`)
      .send({
        phone: 'xxxx',
      })
    ...
    expect(familyMemberResp.statusCode).toBe(201);
    let familyMember = familyMemberResp.body;
    expect(familyMember).toBeDefined();
    expect(familyMember.status).toBe(VdFamilyMemberStatus.WAITING);
    ...
    familyMemberResp = await request(app.getHttpServer())
      ...
      .set('Content-Type', 'application/json')
      .set('Accept', 'application/json');
    expect(familyMemberResp.statusCode).toBe(200);
    familyMember = familyMemberResp.body;
    expect(familyMember).toBeDefined();
    expect(familyMember.status).toBe(VdFamilyMemberStatus.ACCEPTED);
    expect(familyMember.relationship).toBe('Relationship with kids');

    //查看家庭成员列表
    let familyMemberListResp = await request(app.getHttpServer())
      .get(`/virtual-devices/${virtualDevice.id}/family-members`)
      .set('Authorization', `Bearer ${token}`)
      .set('Content-Type', 'application/json')
      .set('Accept', 'application/json');

    expect(familyMemberListResp.statusCode).toBe(200);
    let familyMemberList = familyMemberListResp.body;
    expect(familyMemberList).toBeDefined();
    expect(familyMemberList.length).toBe(1);
    ...
    const deletefamilyMemberResp = await request(app.getHttpServer())
      ...
      .set('Content-Type', 'application/json')
      .set('Accept', 'application/json');
    expect(deletefamilyMemberResp.statusCode).toBe(204);

    //查看家庭成员列表
    familyMemberListResp = await request(app.getHttpServer())
      .get(`/virtual-devices/${virtualDevice.id}/family-members`)
      .set('Authorization', `Bearer ${token}`)
      .set('Content-Type', 'application/json')
      .set('Accept', 'application/json');
    ...
    expect(familyMemberListResp.statusCode).toBe(200);
    familyMemberList = familyMemberListResp.body;
    expect(familyMemberList).toBeDefined();
    expect(familyMemberList.length).toBe(0);
  });
});
